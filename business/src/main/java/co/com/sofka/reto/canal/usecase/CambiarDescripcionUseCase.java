...
public class CambiarDescripcionUseCase extends UseCase<RequestCommand<CambiarDescripcionCommand>, ResponseEvents> {
    @Override
    public void executeUseCase(RequestCommand<CambiarDescripcionCommand> command) {
        var comando = command.getCommand();
        var canal = Canal.from(comando.getCanalId(), repository().getEventsBy(comando.getCanalId().value()));
        canal.cambiarDescripcion(comando.getDescripcion());
